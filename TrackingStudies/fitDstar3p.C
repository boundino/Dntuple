...
const int nBins=4;  Double_t ptBins[nBins+1]={5.,7.,9.,11.,25.};

//TString outputfilename = Form("outputfiles/outputfile%s.root",texData[isData].Data());
void fitDstar3p(Bool_t onlyfit=true)
{
  ...
  void clean0(TH1D* h);  
  TF1* fitDstar(TTree* nt, TTree* ntMC, Double_t ptmin, Double_t ptmax);
  ...
  for(int i=0;i<nBins;i++)
    {
      TF1* fData = fitDstar(ntData,ntMC, ptBins[i],ptBins[i+1]);
  ...
  if(!onlyfit)
    {
      ...
      hPt->SetYTitle(Form("%s Uncorrected 3-prong N(D*)/dp_{T}",texData[isData].Data()));
      hPt->Draw();
      cPt->SaveAs(Form("plots/pp/fitDstar3p/c_%s_Pt.pdf",texData[isData].Data()));
      
      ntMC->Project("hPtMC","Dpt",TCut("%s",weightmc[isData].Data())*Form("%s&&%s",selmc3p[isData].Data(),triggerselection[isData].Data()));
      divideBinWidth(hPtMC);
      ntGen->Project("hPtGen","Gpt",TCut("%s",weightmc[isData].Data())*Form("%s&&%s",selgen3p.Data(),triggerselection[isData].Data()));
...
TF1* fitDstar(TTree* nt, TTree* ntMC, Double_t ptmin, Double_t ptmax)
{
  ...
  TCanvas* c = new TCanvas(Form("c_3p_%d",count),"",600,600);
  TH1D* h = new TH1D(Form("h_3p_%d",count),"",BINNUM,BINMIN,BINMAX);
  TH1D* hMCSignal = new TH1D(Form("hMCSignal_3p_%d",count),"",BINNUM,BINMIN,BINMAX);
  TH1D* hMCSwapped = new TH1D(Form("hMCSwapped_3p_%d",count),"",BINNUM,BINMIN,BINMAX);

  //TF1* f = new TF1(Form("f_3p_%d",count),"[0]*([4]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[4])*Gaus(x,[1],[3])/(sqrt(2*3.14159)*[3]))+(1-exp((0.13957-x)/[7]))*pow(x/0.13957,[8])+[9]*(x/0.13957-1)",BINMIN,BINMAX);
  TF1* f = new TF1(Form("f_3p_%d",count),"[0]*([4]*([6]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[6])*(Gaus(x,[1],[5])/(sqrt(2*3.14159)*[5])))+(1-[4])*Gaus(x,[1],[3])/(sqrt(2*3.14159)*[3]))+(1-exp((0.13957-x)/[7]))*pow(x/0.13957,[8])+[9]*(x/0.13957-1)",BINMIN,BINMAX);
  f->SetLineColor(kRed);
  nt->Project(Form("h_3p_%d",count),"Dmass-DtktkResmass",Form("%s*(%s&&%s&&Dpt>%f&&Dpt<%f)",weightdata[isData].Data(),seldata3p[isData].Data(),triggerselection[isData].Data(),ptmin,ptmax));
  ntMC->Project(Form("hMCSignal_3p_%d",count),"Dmass-DtktkResmass",Form("%s*(%s&&%s&&Dpt>%f&&Dpt<%f)",weightmc[isData].Data(),selmc3p[isData].Data(),triggerselection[isData].Data(),ptmin,ptmax));
  ntMC->Project(Form("hMCSwapped_3p_%d",count),"Dmass-DtktkResmass",Form("%s*(%s&&%s&&Dpt>%f&&Dpt<%f)",weightmc[isData].Data(),selswp3p[isData].Data(),triggerselection[isData].Data(),ptmin,ptmax));
  hMCSignal->Draw();
  f->FixParameter(4,1.);
  f->FixParameter(1,0.145491);
  f->FixParameter(7,1.e+10);
  f->FixParameter(8,1.);
  f->FixParameter(9,0.);
  ...
  hMCSignal->Fit(Form("f_3p_%d",count),"LL","",BINMIN,BINMAX);
  hMCSignal->Fit(Form("f_3p_%d",count),"LL","",BINMIN,BINMAX);
  f->ReleaseParameter(1);
  f->SetParLimits(1,minmass,maxmass);
  hMCSignal->Fit(Form("f_3p_%d",count),"LL","",minmass,maxmass);
  hMCSignal->Fit(Form("f_3p_%d",count),"LL","",minmass,maxmass);
  ...
  hMCSwapped->Fit(Form("f_3p_%d",count),"L q","",BINMIN,BINMAX);
  hMCSwapped->Fit(Form("f_3p_%d",count),"L q","",BINMIN,BINMAX);

  f->FixParameter(4,hMCSignal->Integral(0,1000)/(hMCSwapped->Integral(0,1000)+hMCSignal->Integral(0,1000)));
  f->FixParameter(3,f->GetParameter(3));
  f->ReleaseParameter(7);
  f->SetParLimits(7,0,0.01);
  f->SetParameter(7,0.00389);
  f->ReleaseParameter(8);
  f->SetParLimits(8,0.,10.);
  f->SetParameter(8,0.045);
  f->ReleaseParameter(9);
  f->SetParLimits(9,-1.e+4,1.e+4);
  f->SetParameter(9,-6.2);
  h->Fit(Form("f_3p_%d",count),"LL","",BINMIN,BINMAX);
  h->Fit(Form("f_3p_%d",count),"LL","",BINMIN,BINMAX);
  ...
  TF1* background = new TF1(Form("background_3p_%d",count),"(1-exp((0.13957-x)/[0]))*pow(x/0.13957,[1])+[2]*(x/0.13957-1)");
  ...
  //TF1* mass = new TF1(Form("fmass_3p_%d",count),"[0]*([3]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2]))");
  TF1* mass = new TF1(Form("fmass_3p_%d",count),"[0]*([3]*([5]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[5])*Gaus(x,[1],[4])/(sqrt(2*3.14159)*[4])))");
  ...
  h->Draw("e");
  mass->Draw("same");
  cout<<background->GetParameter(0)<<" "<<background->GetParameter(1)<<" "<<background->GetParameter(2)<<endl;
  background->Draw("same");   
  cout<<f->GetParameter(0)<<" "<<f->GetParameter(1)<<" "<<f->GetParameter(2)<<" "<<f->GetParameter(3)<<" "<<f->GetParameter(4)<<" "<<f->GetParameter(5)<<" "<<f->GetParameter(6)<<" "<<f->GetParameter(7)<<" "<<f->GetParameter(8)<<" "<<f->GetParameter(9)<<endl;
  f->Draw("same");
  ...
  c->SaveAs(Form("plots/pp/fitDstar3p/DMass_%s-%d.pdf",texData[isData].Data(),count));
