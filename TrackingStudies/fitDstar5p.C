  ...
  if(!onlyfit)
    {
      ...
      hPt->SetYTitle(Form("%s Uncorrected 5-prong dN(D*)/dp_{T}",texData[isData].Data()));
      hPt->Draw();
      cPt->SaveAs(Form("plots/pp/fitDstar5p/c_%s_Pt.pdf",texData[isData].Data()));
      
      TH1D* hPtScl = (TH1D*)hPt->Clone("hPtScl");
      hPtScl->Scale(scaleMC5p[isData]);
      TCanvas* cPtScl =  new TCanvas("cPtScl","",600,600);
      cPtScl->SetLogy();
      hPtScl->SetXTitle("D p_{T} (GeV/c)");
      hPtScl->SetYTitle(Form("%s Scaled 5-prong dN(D*)/dp_{T}",texData[isData].Data()));
      hPtScl->Draw();
      cPtScl->SaveAs(Form("plots/pp/fitDstar5p/c_%s_PtScl.pdf",texData[isData].Data()));

      ntMC->Project("hPtMC","Dpt",TCut("%s",weightmc[isData].Data())*Form("%s&&%s",selmc5p[isData].Data(),triggerselectionmc[isData].Data()));
      divideBinWidth(hPtMC);
      ntGen->Project("hPtGen","Gpt",TCut("%s",weightmc[isData].Data())*Form("%s&&%s",selgen5p.Data(),triggerselectionmc[isData].Data()));
      divideBinWidth(hPtGen);  
      
      TH1D* hEff = (TH1D*)hPtMC->Clone("hEff");
      hEff->SetTitle(Form(";D p_{T} (GeV/c);%s 5-prong Efficiency",texData[isData].Data()));
      hEff->Divide(hPtGen);
      TCanvas* cEff = new TCanvas("cEff","",600,600);
      hEff->Draw();
      cEff->SaveAs("plots/pp/fitDstar5p/cEff.pdf");
      
      TH1D* hPtCor = (TH1D*)hPt->Clone("hPtCor");
      hPtCor->SetTitle(Form(";D p_{T} (GeV/c);%s Corrected 5-prong dN(D*)/dp_{T}",texData[isData].Data()));
      hPtCor->Divide(hEff);
      ...
      cPtCor->SaveAs(Form("plots/pp/fitDstar5p/c_%s_PtCor.pdf",texData[isData].Data()));

      TFile* outputfile = new TFile(Form("outputfiles/output_5p_%s_hPtScl.root",texData[isData].Data()),"recreate");
      outputfile->cd();
      hPt->Write();
      hPtScl->Write();
      hEff->Write();
...
TF1* fitDstar(TTree* nt, TTree* ntMC, Float_t ptmin, Float_t ptmax, Bool_t plotgenmatch)
{
  TCanvas* c = new TCanvas(Form("c_5p_%.0f_%.0f",ptmin,ptmax),"",600,600);
  TH1D* h = new TH1D(Form("h_5p_%.0f_%.0f",ptmin,ptmax),"",BINNUM,BINMIN,BINMAX);
  ...
  TF1* background = new TF1(Form("background_5p_%.0f_%.0f",ptmin,ptmax),"[3]*((1-exp((0.13957-x)/[0]))*pow(x/0.13957,[1])+[2]*(x/0.13957-1))");
  ...
  TF1* massSwap = new TF1(Form("fmassSwap_5p_%.0f_%.0f",ptmin,ptmax),"[0]*(1-[2])*Gaus(x,[1],[3])/(sqrt(2*3.14159)*[3])");
  ...
  if(plotgenmatch&&(isData==MC||isData==MC_MB))
    {
  ...
  c->SaveAs(Form("plots/pp/fitDstar5p/DMass_%s_%.0f_%.0f.pdf",texData[isData].Data(),ptmin,ptmax));
