...
TF1* fit(TTree* nt, TTree* ntMC, double ptmin, double ptmax)
{
   ...
   ntMC->Project(Form("hMCSwapped%d",count),"dcandmass",Form("%s*(%s&&dcandpt>%f&&dcandpt<%f&&(matchedtogen&&!nongendoublecounted))",weight.Data(),seldata.Data(),ptmin,ptmax));
   ...
   if(nBins==1) c->SaveAs("../ResultsD0purdue/DMass-inclusive.pdf");
   else c->SaveAs(Form("../ResultsD0purdue/DMass-%d.pdf",count));
