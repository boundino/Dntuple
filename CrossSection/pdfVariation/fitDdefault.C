...
void fitDdefault(TString collsyst="PbPb", TString outputfile="outfMasshisto/mass")
{
...
TF1* fit(Double_t ptmin, Double_t ptmax)
{
  ...
  TF1* f = new TF1(Form("f_%.0f_%.0f",ptmin,ptmax),"[0]*([7]*([9]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[9])*Gaus(x,[1],[10])/(sqrt(2*3.14159)*[10]))+(1-[7])*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))+[3]+[4]*x+[5]*x*x+[6]*x*x*x", 1.7, 2.0);
  ...
  c->SaveAs(Form("plotFits/DMass_default_%s_%.0f_%.0f.pdf",collisionsystem.Data(),ptmin,ptmax));
  ...
  lPull->Draw();
  cPull->cd();
  cPull->SaveAs(Form("plotFits/DMass_default_%s_%.0f_%.0f_Pull.pdf",collisionsystem.Data(),ptmin,ptmax));
