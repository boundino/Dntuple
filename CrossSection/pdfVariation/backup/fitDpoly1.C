...
void fitDpoly1(TString collsyst="PbPb", TString outputfile="outfMasshisto/mass")
{
  ...
  ofstream fout(Form("outYield/poly1_%s.dat",collsyst.Data()));
...
TF1* fit(Double_t ptmin, Double_t ptmax)
{
  ...
  TF1* f = new TF1(Form("f_%.0f_%.0f",ptmin,ptmax),"[0]*([7]*([9]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[9])*Gaus(x,[1],[10])/(sqrt(2*3.14159)*[10]))+(1-[7])*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))+[3]+[4]*x", 1.7, 2.0);
  ...
  TF1* background = new TF1(Form("background_%.0f_%.0f",ptmin,ptmax),"[0]+[1]*x");
