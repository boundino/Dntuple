...
void fitDincreasewid(TString collsyst="PbPb", TString outputfile="outfMasshisto/mass")
{
  ...
  ofstream fout(Form("outYield/increasewid_%s.dat",collsyst.Data()));
...
TF1* fit(Double_t ptmin, Double_t ptmax)
{
  ...
  c->SaveAs(Form("plotFits/DMass_increasewid_%s_%.0f_%.0f.pdf",collisionsystem.Data(),ptmin,ptmax));
  ...
  lPull->Draw();
  cPull->cd();
  cPull->SaveAs(Form("plotFits/DMass_increasewid_%s_%.0f_%.0f_Pull.pdf",collisionsystem.Data(),ptmin,ptmax));
